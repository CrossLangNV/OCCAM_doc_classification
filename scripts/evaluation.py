...
from classifier.data import gen_im_paths, image_preprocessing
from classifier.datasets import BOG, Subset, NBB
...
def eval_NBB(model_NBB,
             verbose: int = 1) -> dict:
    return eval_shared(model_NBB, FOLDER_TEST_NBB, label='NBB', index=0, verbose=verbose)


def eval_DH(model,
            verbose:int=1) -> dict:
    FOLDER_DH_1 = os.path.join(ROOT, r'data/preprocessed/handwritten/test')
    FOLDER_DH_2 = os.path.join(ROOT, r'data/preprocessed/newspapers/test')
    FOLDER_DH_3 = os.path.join(ROOT, r'data/preprocessed/printed/test')
    eval_shared(model, FOLDER_DH_1, label='handwritten', index=0, verbose=verbose)
    eval_shared(model, FOLDER_DH_2, label='newspapers', index=0, verbose=verbose)
    eval_shared(model, FOLDER_DH_3, label='printed', index=0, verbose=verbose)
...
def eval_BOG_VS_NBB(model,
    ...
    x_BOG = BOG(Subset.TEST)
    x_NBB = NBB(Subset.TEST)
...
def eval_shared(model: DocModel,
    ...
    for i, filename in enumerate(gen_im_paths(dir)):
        ...
        im = image_preprocessing(Image.open(filename), shape=(IMAGE_WIDTH, IMAGE_WIDTH))
